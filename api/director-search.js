...
export default async function handler(req, res) {
  try {
    ...
    let productFilters = '';
    if (q) productFilters += `&search_text=ilike.*${esc(q)}*`;
    if (brand) productFilters += `&brand=ilike.${esc(brand)}`;
    if (gender) productFilters += `&gender=ilike.${esc(gender)}`;
    if (type) productFilters += `&fragrance_type=ilike.${esc(type)}`;

    const rows = await db(`store_products?store_id=eq.${storeId}&published=eq.true&select=product_id,products(canonical_name,brand,gender,fragrance_type,size_ml,image_url,search_text),supplier_products(supplier_sku,supplier_price,in_stock)&limit=100${productFilters}`);
    ...
    }).filter(p => p.inStock && p.salePrice != null && (!maxPrice || p.salePrice <= maxPrice)).sort((a,b) => a.salePrice-b.salePrice).slice(0,limit);
