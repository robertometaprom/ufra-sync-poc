...
async function authUser(req){const h=String(req.headers.authorization||'').trim(),m=h.match(/^Bearer\s+(.+)$/i);if(!m)return null;const{url,key}=cfg();const r=await fetch(`${url}/auth/v1/user`,{headers:{apikey:key,Authorization:`Bearer ${m[1]}`},cache:'no-store'});const u=await r.json().catch(()=>null);return r.ok&&u?.id?u:null}
...
export default async function handler(req,res){
  try{
    ...
    const orders=await db(`orders?id=eq.${orderId}&select=id,user_id,status,payment_status,total,currency,customer_email&limit=1`),order=orders?.[0];if(!order)return res.status(404).json({ok:false,error:'Order not found'});if(order.user_id){const user=await authUser(req);if(!user||user.id!==order.user_id)return res.status(403).json({ok:false,error:'Order access denied'})}if(order.status!=='pending_payment'||order.payment_status!=='pending')return res.status(409).json({ok:false,error:'Order is not pending payment'});
