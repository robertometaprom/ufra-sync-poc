...
<div id="shxAuthBackdrop" class="shx-auth-backdrop">
  <div class="shx-auth-card" role="dialog" aria-modal="true" aria-label="Mi cuenta SHAXXIA">
    ...
    <div class="shx-auth-guest">
      <h2>Mi cuenta</h2><p>Entra o crea tu cuenta con tu correo para guardar tu acceso y tus pedidos.</p>
    ...
    <div class="shx-auth-signed" hidden><h2>Mi cuenta</h2><p>Sesión iniciada correctamente.</p><div class="shx-auth-user"></div><button class="shx-auth-logout" type="button">Cerrar sesión</button></div>
  </div>
</div>
...
try{
  ...
  function renderAuth(s){const u=s?.user;if(u){accountBtn.textContent=u.email||'Mi cuenta';guest.hidden=true;signed.hidden=false;userBox.textContent=u.email||'Cuenta activa'}else{accountBtn.textContent='Entrar';guest.hidden=false;signed.hidden=true;userBox.textContent=''}}
  function openAuth(){status.textContent='';backdrop.classList.add('open');currentSession().then(renderAuth)}function closeAuth(){backdrop.classList.remove('open')}
  ...
  window.SHAXXIA_AUTH={async getAccessToken(){return (await currentSession())?.access_token||null},async getUser(){return (await currentSession())?.user||null},open:openAuth};currentSession().then(renderAuth);
