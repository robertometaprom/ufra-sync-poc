...
.shx-account-btn{position:fixed;top:14px;right:18px;z-index:80;border:1px solid rgba(35,28,23,.18);background:#fffaf5;color:#1d1815;border-radius:999px;padding:10px 14px;font:600 13px/1.1 Arial,sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.09);cursor:pointer;max-width:210px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
@media(max-width:720px){.shx-account-btn{top:10px;right:10px;padding:9px 12px}.shx-auth-actions{grid-template-columns:1fr}}
</style>
<script>
try {
  ...
  /* Customer login: direct Supabase Auth with publishable key. No service secret is exposed. */
  ...
  function saveAuth(v){
    if(v)localStorage.setItem(AUTH_KEY,JSON.stringify(v)); else localStorage.removeItem(AUTH_KEY);
  }
  ...
  async function refreshSession(){
    ...
    try{
      const data=await authFetch('/auth/v1/token?grant_type=refresh_token',{method:'POST',body:JSON.stringify({refresh_token:s.refresh_token})});
      const next={access_token:data.access_token,refresh_token:data.refresh_token,user:data.user,expires_at:Date.now()+((data.expires_in||3600)*1000)};
  ...
  async function currentSession(){
    let s=loadAuth();
    if(!s)return null;
    if(!s.expires_at||Date.now()>s.expires_at-60000)s=await refreshSession();
    return s;
  }

  const accountBtn=document.createElement('button');
  accountBtn.className='shx-account-btn';
  ...
  const guest=backdrop.querySelector('.shx-auth-guest');
  const signed=backdrop.querySelector('.shx-auth-signed');
  const status=backdrop.querySelector('.shx-auth-status');
  const email=backdrop.querySelector('.shx-auth-email');
  const password=backdrop.querySelector('.shx-auth-password');
  const userBox=backdrop.querySelector('.shx-auth-user');

  function renderAuth(session){
    const u=session?.user;
    if(u){
      accountBtn.textContent=u.email||'Mi cuenta';
      guest.hidden=true;signed.hidden=false;
      userBox.textContent=u.email||'Cuenta activa';
    }else{
      accountBtn.textContent='Entrar';
      guest.hidden=false;signed.hidden=true;
      userBox.textContent='';
    }
  }
  ...
  backdrop.querySelector('.shx-auth-close').addEventListener('click',closeAuth);
  backdrop.addEventListener('click',e=>{if(e.target===backdrop)closeAuth()});

  async function signIn(){
    status.textContent='';
    const e=email.value.trim(),p=password.value;
    if(!e||!p){status.textContent='Escribe tu correo y contraseña.';return}
    try{
      const data=await authFetch('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email:e,password:p})});
      ...
      saveAuth(session);renderAuth(session);
    }catch(err){status.textContent=err.message}
  }
  async function signUp(){
    status.textContent='';
    const e=email.value.trim(),p=password.value;
    if(!e||!p){status.textContent='Escribe tu correo y una contraseña.';return}
    if(p.length<6){status.textContent='La contraseña debe tener al menos 6 caracteres.';return}
    try{
      const data=await authFetch('/auth/v1/signup',{method:'POST',body:JSON.stringify({email:e,password:p,data:{brand:'SHAXXIA'}})});
      if(data?.access_token){
        const session={access_token:data.access_token,refresh_token:data.refresh_token,user:data.user,expires_at:Date.now()+((data.expires_in||3600)*1000)};
        saveAuth(session);renderAuth(session);
      }else{
        status.style.color='#365a38';
        status.textContent='Cuenta creada. Revisa tu correo para confirmar tu cuenta y después entra aquí.';
      }
    }catch(err){status.textContent=err.message}
  }
  backdrop.querySelector('.shx-auth-primary').addEventListener('click',signIn);
  backdrop.querySelector('.shx-auth-secondary').addEventListener('click',signUp);
  backdrop.querySelector('.shx-auth-logout').addEventListener('click',async()=>{
    const s=loadAuth();
    try{if(s?.access_token)await authFetch('/auth/v1/logout',{method:'POST',headers:{Authorization:'Bearer '+s.access_token},body:'{}'})}catch{}
    saveAuth(null);renderAuth(null);closeAuth();
  });
  password.addEventListener('keydown',e=>{if(e.key==='Enter')signIn()});

  window.SHAXXIA_AUTH={
    async getAccessToken(){return (await currentSession())?.access_token||null},
    async getUser(){return (await currentSession())?.user||null},
    open:openAuth
  };
